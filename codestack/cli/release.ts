...
/**
 * codestack CLI release script.
 *
 * Triggers the codestack-release.yml GitHub Actions workflow
 * to build, publish, and release the codestack CLI to npm.
 *
 * Usage:
 *   bun codestack/cli/release.ts [patch|minor|major] [--ref <commit-sha>]
 *
 * Requires:
...
async function triggerWorkflow(versionType: string, checkoutRef: string) {
  ...
  try {
    ...
    const triggerCmd = `curl -s -w "HTTP Status: %{http_code}" -X POST \
      ...
      https://api.github.com/repos/CodebuffAI/codebuff/actions/workflows/codestack-release.yml/dispatches \
      -d '${payload}'`
    ...
    if (response.includes('workflow_dispatch')) {
      ...
      log(
        'Please manually trigger the workflow at: https://github.com/CodebuffAI/codebuff/actions/workflows/codestack-release.yml',
      )
    } else {
      log('🎉 codestack release workflow triggered!')
    }
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : String(err)
    log(`⚠️  Failed to trigger workflow automatically: ${message}`)
    log(
      'You may need to trigger it manually at: https://github.com/CodebuffAI/codebuff/actions/workflows/codestack-release.yml',
    )
  }
}

async function main() {
  log('🚀 Initiating codestack release...')
  ...
  log('')
  log(
    'Monitor progress at: https://github.com/CodebuffAI/codebuff/actions/workflows/codestack-release.yml',
  )
}
