...
function createConfig(packageName) {
  ...
  return {
    ...
    metadataPath: path.join(configDir, 'codestack-metadata.json'),
    tempDownloadDir: path.join(configDir, '.codestack-download-temp'),
...
async function downloadBinary(version) {
  ...
  term.clearLine()
  console.log('Download complete! Starting codestack...')
}
...
async function ensureBinaryExists() {
  ...
  try {
    ...
    term.clearLine()
    console.error('❌ Failed to download codestack:', error.message)
    console.error('Please check your internet connection and try again')
    if (!getProxyUrl()) {
      console.error(
        'If you are behind a proxy, set the HTTPS_PROXY environment variable',
  ...
  try {
    ...
    if (
      ...
      newChild.on('error', (err) => {
        console.error('Failed to start codestack:', err.message)
        process.exit(1)
      })
