...
export class codestackSession {
  ...
  /**
   * Start a codestack binary in a tmux session.
  ...
  static async start(
  ...
  ): Promise<codestackSession> {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'codestack-e2e-'))

    // Create a minimal project so codestack has something to work with
    fs.writeFileSync(
    ...
    return new codestackSession(sessionName, tmpDir)
  }
  ...
  /** Send text input to the codestack CLI (presses Enter by default). */
  async send(
    text: string,
