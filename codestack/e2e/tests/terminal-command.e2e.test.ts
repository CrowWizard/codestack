/**
 * E2E test that verifies codestack can run terminal commands.
 *
 * Starts codestack in tmux, sends a prompt asking it to run a shell command,
...
describe('codestack: Terminal Command', () => {
  ...
  test(
    ...
    async () => {
      ...
      session = await codestackSession.start(binary, { waitSeconds: 5 })

      // Wait for the CLI to be fully ready before sending input
      await session.waitForReady()

      // Ask codestack to run a shell command whose output can only come from
